...
from SAM.sam_queue import SAMQueue

def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, gmm: GMM=None, max_norm: float = 0):
    model.train()
    criterion.train()
...
def train_one_epoch_semi(model_student: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, gmm: GMM=None, sam_queue: SAMQueue=None, max_norm: float = 0, threshold: float = 0.7, ema_keep_rate: float = 0.9996, passed_num_path: str = '', train_setting: str = '', pixels: int = 800, w_p: float = 0.5, w_t: float = 0.5):
    model_student.train()
    model_teacher.eval()
    criterion.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    for _ in metric_logger.log_every(range(len(data_loader_label)), print_freq, header):
        ...
        # print("tensor_unlabel_k", tensor_unlabel_k)
        # print("tensor_unlabel_k.dim(): ", tensor_unlabel_k.dim())
        samples = []
        ...
        with torch.no_grad():
            ...
            teacher_1 = time.time()
            #print("forward pass teacher: ", teacher_1-teacher_0)
            
            filtering_0 = time.time()
            # we need to undo this and save the image after transformations applied to the teacher _k images
            mean = torch.tensor([0.485, 0.456, 0.406])
            std = torch.tensor([0.229, 0.224, 0.225])
            normalized_tensor = tensor_unlabel_k.clone()  # Create a copy of the tensor to avoid modifying the original
            for t, m, s in zip(normalized_tensor, mean, std):
                t.mul_(s).add_(m)

            to_pil = T.ToPILImage()
            unlabel_k_pil = to_pil(normalized_tensor.cpu()) # Make sure to move the tensor to CPU if it's on the GPU

            # unlabel_k_pil.save("unlabel_k_pil.png")
            # update GMM
            if gmm:
                gmm.score_update(outputs_unsup_q)

        # add compute thresholds for each class
        if gmm:
            threshold = thresholds = gmm.get_current_gmm_thresholds()
            thresholds = torch.tensor(thresholds).to(device)
        else:
            # print("Current thrs: ", thresholds)
            threshold = 0.7

        # pseudo label filtering
        if train_setting == 'dvd':
            pseudo_label_k = unified_filter_pseudo_labels(outputs_unsup_q, targets_unlabel_q, targets_unlabel_k,
                                                          records_unlabel_q, records_unlabel_k, pixels, sam_queue, unlabel_k_pil,
                                                          label_type='Unsup', w_p=0.5, w_t=0.5, is_binary=False,
                                                          thresholds=threshold)
            
        filtering_1 = time.time()
        #print("Filtering + SAM: ", filtering_1-filtering_0)
...
def train_one_epoch_burnin(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    if gmm:
        ...
        gmm.gmm_recompute()
        t1 = time.time()
        print("gmm_recompute() time: ", t1-t0)
