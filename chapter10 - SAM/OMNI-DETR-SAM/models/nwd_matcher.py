...
import numpy as np
# Jacob
from util.gaussians import gaussian_loss_matcher_v2 # class gaussians
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            #                                   box_cxcywh_to_xyxy(tgt_bbox))  # 600 * 
            
            # Compute the gaussian loss NWD
            cost_giou = gaussian_loss_matcher_v2(out_bbox, tgt_bbox)    
...
class HungarianMatcherSemi(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
...
def build_nwd_matcher(args):
    return HungarianMatcher(cost_class=args.set_cost_class,
                            cost_bbox=args.set_cost_bbox,
                            cost_giou=args.set_cost_giou)

def build_nwd_matcher_semi(args):
