...
def unified_filter_pseudo_labels(pseudo_unsup_outputs, targets_unlabel_q, targets_unlabel_k, records_unlabel_q, records_unlabel_k, pixels, sam:SAM, imgs_unlabel, label_type,  w_p=0.5, w_t=0.5, is_binary=False, thresholds=None):
    pseudo_unsup_logits = pseudo_unsup_outputs['pred_logits']
    softmax = nn.Softmax(dim=2)
    pseudo_unsup_prob = softmax(pseudo_unsup_logits)

    # print("pseudo_unsup_prob.shape ", pseudo_unsup_prob.shape)
    # print("pseudo_unsup_prob: ", pseudo_unsup_prob)
    ...
    if is_binary:
        ...
        if label_type == 'Unsup':
            # # print("THRESHOLDS:")
            # print(thresholds)
            # # Get the maximum probability and its corresponding index along the last dimension
            # classes_scores, classes_indices = torch.max(pseudo_unsup_prob, dim=2)
            # # print("classes_scores")
            # # print(classes_scores)
            # # print("classes_indices")
            # # print(classes_indices)
            # #print(" [BEFORE] classes_indices.shape: ", classes_indices.shape[1])
            # # NOTE: we need to guard agains larger values than num_classes due to our setting. However class-predictions larger than num_classe should never be a pseudo_label!
            # num_classes = 10  # Replace with your desired constant threshold
            # mask = (classes_indices <= num_classes).nonzero(as_tuple=True)
            # #print("[MASK]")
            # # print(mask)
            # #  Apply the mask to filter class indices and scores
            # classes_indices = classes_indices[mask]
            # #print("[AFTER]  classes_indices.shape: ", classes_indices.shape[0])
            # classes_scores = classes_scores[mask]
            # # reshape them for magic
            # classes_indices = torch.reshape(classes_indices, (1, classes_indices.shape[0]))
            # classes_scores = torch.reshape(classes_scores, (1, classes_scores.shape[0]))
            # #classes_scores = classes_scores[mask]

            # #print("classes_scores.shape ", classes_scores.shape)
            # #print("classes_indices")
            # #print(classes_indices)
            # #print("classes_indices.shape")
            # #print(classes_indices.shape) # torch.Size([1, 900])
            # # get the corresponding thresholds in the correct shape of the class-preidctions
            # # Create a new tensor with threshold values by indexing into thresholds using class_predictions
            # #print("thresholds: ", thresholds)
            # index_wise_threshold = thresholds[classes_indices]
            # # print("index_wise_threshold: ", index_wise_threshold)

            # # # Create a mask tensor based on the class-specific thresholds
            # # mask = max_probs > thresholds[class_indices]
            # # Create a mask tensor based on the class-specific thresholds
        
            # satisfied_idx = (classes_scores > index_wise_threshold).nonzero(as_tuple=True)
            # #print("satisfied_idx: ", satisfied_idx)
            # # satisfied_idx = (classes_scores > threshold).nonzero(as_tuple=True)
            
            # #print("pseudo_unsup_outputs['pred_boxes']")
            # #print(pseudo_unsup_outputs['pred_boxes'])

            # pseudo_unsup_bbox = pseudo_unsup_outputs['pred_boxes']
            # satisfied_bbox = pseudo_unsup_bbox[satisfied_idx[0], satisfied_idx[1], :]
            # satisfied_class = classes_indices[satisfied_idx[0], satisfied_idx[1]]
            # # satisfied_bbox = pseudo_unsup_bbox[satisfied_idx]
            # # satisfied_class = classes_indices[satisfied_idx]
            ...
            th = 0.7
            classes_scores, classes_indices = torch.max(pseudo_unsup_prob, dim=2)
            satisfied_idx = (classes_scores > th).nonzero(as_tuple=True)
            ...
            # SAM OPERATIONS 
            # Get SAM Predictions 
            # It can be the case, that bbox doesn't survive. Therefore we remove that box and cls totally. 
            if sam is not None:
                satisfied_bbox_2, satisfied_class_2, satisfied_idx_2 = sam.replace_boxes_by_sam(imgs_unlabel, satisfied_bbox, satisfied_class)
                # # replace the old onces, with the new onces
                satisfied_bbox = satisfied_bbox_2
                satisfied_class = satisfied_class_2
                satisfied_idx = satisfied_idx_2
                # in the futue lets plots this for sanity
            
            """
            Note all our index is satisfied below. 
            """

    # print("satisfied_bbox_2: ", satisfied_bbox_2)
    # print("satisfied_class_2: ", satisfied_class_2)
    # print("satisfied_idx_2: ", satisfied_idx_2)
    if sam is not None:
        pseudo_annotations = []
        #print("pseudo_unsup_bbox.shape: ", pseudo_unsup_bbox.shape)
        for i in range(pseudo_unsup_bbox.shape[0]): # iterate over batch?
            i_target = targets_unlabel_q[i]
            i_dict = {}
            #print("satisfied_idx: ", satisfied_idx)
            #i_idx = (satisfied_idx[0] == i).nonzero(as_tuple=True)
            # i_idx = satisfied_idx[0]
            # print("i: ", i)
            # #print("satisfied_idx[0]: ", satisfied_idx[0])
            # print("i_idx: ", i_idx[0])
            i_idx, _ = satisfied_idx

            if len(i_idx) == 0:  # means no predicted bbox satisfies the confidence threshold
                # just assign []
                ...
                i_boxes = satisfied_bbox
                i_labels = satisfied_class
                cur_i_idx = torch.from_numpy(i_idx).to('cuda')
                
                # box-calibration is to apply app the augmentations to the current boxes
