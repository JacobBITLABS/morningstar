...
def train_one_epoch_semi(model_student: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module, vanilla_criterion: torch.nn.Module,
    ...
    prefetcher_label = data_prefetcher_semi(data_loader_label, device, prefetch=True)  # data_loader_label is the hard labels
    prefetcher_unlabel = data_prefetcher_semi(data_loader_unlabel, device, prefetch=True)  # data_loader_unlabel are the pseudo labels. They're only generated after passing through the teacher and pseudo-label-filter
    ...
    for _ in metric_logger.log_every(tqdm(range(len(data_loader_label)), desc="Training semi"), print_freq, header):
        ...
        if train_setting == 'coco_add_semi' or train_setting == 'voc_semi':
            pseudo_label_k = unified_filter_pseudo_labels(outputs_unsup_q, targets_unlabel_q, targets_unlabel_k, records_unlabel_q, records_unlabel_k, pixels, label_type='Unsup', w_p=0.5, w_t=0.5, is_binary=False, threshold=threshold)
        ...
        # cutting it again
        cutter = len(targets_label_q) + len(targets_label_k[0])
        loss_dict = criterion(outputs[:cutter], targets[:cutter], indicators[:cutter])
        weight_dict = criterion.weight_dict
        losses = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)

        # pseudo_losses
        loss_dict_pseudo = vanilla_criterion(outputs[cutter:], targets[cutter:], indicators[:cutter])
        weight_dict = vanilla_criterion.weight_dict
        losses_pseudo = sum(loss_dict_pseudo[k] * weight_dict[k] for k in loss_dict_pseudo.keys() if k in weight_dict) 
        
        # TODO: Make sure the next line is evaulated to be a scalar
        losses += losses_pseudo

        # TODO: Make sure this take BOTH weight dicts into account
        # reduce losses over all GPUs for logging purposes
        loss_dict_reduced = utils.reduce_dict(loss_dict)
        loss_dict_reduced_unscaled = {f'{k}_unscaled': v
                                      for k, v in loss_dict_reduced.items()}
...
def train_one_epoch_burnin(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    # for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
    #for _ in metric_logger.log_every(tqdm(range(len(data_loader)), desc="Training burn-in"), print_freq, header):
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        outputs = model(samples)
        loss_dict = criterion(outputs, targets)
        weight_dict = criterion.weight_dict
