...
from .segmentation import (DETRsegm, PostProcessPanoptic, PostProcessSegm,
                           dice_loss, sigmoid_focal_loss, sigmoid_weighted_focal_loss, vanilla_sigmoid_weighted_focal_loss)
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # print("No: TARGET LOSS LABELS: ", len(targets))
        ...
        # Handle LAM Mask
        lam_target_classes_o = torch.cat([t["perspective_mask"][J] for t, (_, J) in zip(targets, indices)])
        #print("lam_target_classes_o: ", lam_target_classes_o)
        FILL_VALUE = 1.0 # we want each elementwise loss to be multiplied with 1, by default. Otherwise lam or 1-lam
        lam_target_classes = torch.full(src_logits.shape[:2], FILL_VALUE, dtype=torch.float32, device=src_logits.device)
        lam_target_classes[idx] = lam_target_classes_o

        # torch.set_printoptions(threshold=10_000) # this will make the whole tensor show in std.out.
        # print("lam_target_classes_o: ", lam_target_classes)

        loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_boxes, lam_target_classes, alpha=self.focal_alpha, gamma=2) * \
                  src_logits.shape[1]
...
class SetCriterion_semi(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, indicators, log=True):
        ...
        FILL_VALUE = 1.0 # we want each elementwise loss to be multiplied with 1, by default. Otherwise lam or 1-lam
        lam_target_classes = torch.full(src_logits.shape[:2], FILL_VALUE, dtype=torch.float32, device=src_logits.device)
        lam_target_classes[idx] = lam_target_classes_o  # shape: (900, 20)
        ...
        loss_ce = sigmoid_weighted_focal_loss(src_logits, target_classes_onehot, lam_target_classes, num_boxes, empty_indicators,
    ...
    def forward(self, outputs, targets, indicators=None):
        ...
        # matcher. Assignement of each predictions against possible targets        
        indices = self.matcher(outputs_without_aux, targets)
...
"""
VANILLA SEMI CRITERION
"""
class Vanilla_SetCriterion_semi(nn.Module):
    """ This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, indicators, log=True):
        ...
        loss_ce = vanilla_sigmoid_weighted_focal_loss(src_logits, target_classes_onehot, num_boxes, empty_indicators,
                                              alpha=self.focal_alpha, gamma=2) * src_logits.shape[1]
        losses = {'loss_ce': loss_ce}
...
def build_semi(args):
    ...
    criterion.to(device)
    vanilla_criterion = Vanilla_SetCriterion_semi(num_classes, matcher, weight_dict, losses, focal_alpha=args.focal_alpha)
    criterion.to(device)
    ...
    return model_student, model_teacher, criterion, criterion_burnin, vanilla_criterion, postprocessors
