...
from engine import evaluate, train_one_epoch_semi, train_one_epoch_burnin #train_one_epoch,
...
def get_args_parser():
    ...
    parser.add_argument('--annotation_json_unlabel', default='',
                        help='percent with fully labeled')
    parser.add_argument('--annotation_json_aerial_label', default='',
                        help='percent with fully labeled')
    parser.add_argument('--annotation_json_aerial_unlabel', default='',
                        help='percent with fully labeled')
    parser.add_argument('--annotation_json_ground_label', default='',
                        help='percent with fully labeled')
    parser.add_argument('--annotation_json_ground_unlabel', default='',
                        help='percent with fully labeled')
...
def main(args):
    ...
    # TODO: Entry point where the model(s) are instantiated. Start Tracing from here to find where you need to edit the loss functions (probably at around line 200 of engine.py)
    if args.dataset_file == "coco":
        ...
        model_student, model_teacher, criterion, criterion_burnin, vanilla_criterion, postprocessors= build_model_semi(args)
    ...
    # TODO: Entry point where the dataset and all of its augmentations are set. Start tracing from here to find where you need to apply mixup(probably in the __getitem__ function of the CocoDetection_semi class)
    if args.dataset_file == "coco":
        ...
        if epoch < args.BURN_IN_STEP:
            ...
            if (epoch + 1) % args.eval_freq == 0:
                ...
                test_stats, coco_evaluator = evaluate(
                    model_student, vanilla_criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir
                )
            ...
            train_stats = train_one_epoch_semi(
                model_student, model_teacher, criterion, vanilla_criterion, data_loader_train_label, data_loader_train_unlabel, optimizer,
                device, epoch,
            ...
            if (epoch + 1) % args.eval_freq == 0:
                ...
                test_stats_teacher, coco_evaluator = evaluate(
                    model_teacher, vanilla_criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir
                )
