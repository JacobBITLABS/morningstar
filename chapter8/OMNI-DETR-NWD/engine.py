...
def train_one_epoch_semi(model_student: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, gmm: GMM=None, sam: SAM=None, max_norm: float = 0, threshold: float = 0.7, ema_keep_rate: float = 0.9996, passed_num_path: str = '', train_setting: str = '', pixels: int = 800, w_p: float = 0.5, w_t: float = 0.5):
    model_student.train()
    model_teacher.eval()
    criterion.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    for _ in metric_logger.log_every(range(len(data_loader_label)), print_freq, header):
        ...
        if train_setting == 'dvd':
            pseudo_label_k = unified_filter_pseudo_labels(outputs_unsup_q, targets_unlabel_q, targets_unlabel_k,
                                                          records_unlabel_q, records_unlabel_k, pixels, sam, unlabel_k_pil,
