...
import copy
# Jacob
#from util.gaussians import gaussian_loss
#from .nwd_matcher import build_nwd_matcher, build_nwd_matcher_semi
...
#         loss_giou, _ = gaussian_loss(src_boxes, target_boxes)
#         losses['loss_giou'] = loss_giou.sum() / num_boxes
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        # loss_giou, loss_giou_lst = gaussian_loss(src_boxes, target_boxes)
        # losses['loss_giou'] = loss_giou.sum() / num_boxes
...
class SetCriterion_semi(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, indicators):
        ...
        if len(batch_idx) == 0:
            ...
            # _, loss_giou_lst = gaussian_loss(src_boxes, target_boxes)
            # loss_giou = loss_giou_lst * indicator_weights
            # losses['loss_giou'] = loss_giou.sum() / num_boxes
...
def build_semi(args):
    ...
    #matcher = build_nwd_matcher_semi(args) # NWD MATCHER
    weight_dict = {'loss_ce': args.cls_loss_coef, 'loss_bbox': args.bbox_loss_coef}
    weight_dict['loss_giou'] = args.giou_loss_coef
    if args.masks:
        weight_dict["loss_mask"] = args.mask_loss_coef
