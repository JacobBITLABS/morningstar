...
def unified_filter_pseudo_labels(pseudo_unsup_outputs, targets_unlabel_q, targets_unlabel_k, records_unlabel_q, records_unlabel_k, pixels, label_type, w_p=0.5, w_t=0.5, is_binary=False, threshold=0.7, thresholds=None):
    ...
    if is_binary:
        ...
        if label_type == 'Unsup':
            ...
            # Convert class_ids_tensor to a flat tensor
            class_ids_flat = classes_indices.flatten()
            # Replace class IDs with corresponding threshold values using torch.take
            result_tensor = torch.take(torch.tensor(thresholds).to('cuda'), class_ids_flat)
            # Reshape the result_tensor to match the original shape of class_ids_tensor
            index_wise_threshold = result_tensor.view(classes_indices.size())
            # Print or use the result_tensor as needed
            # print("result_tensor")
            # print(result_tensor)
