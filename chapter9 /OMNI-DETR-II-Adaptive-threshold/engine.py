...
def train_one_epoch_semi(model_student: torch.nn.Module, model_teacher: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, gmm: GMM=None, max_norm: float = 0, threshold: float = 0.7, ema_keep_rate: float = 0.9996, passed_num_path: str = '', train_setting: str = '', pixels: int = 800, w_p: float = 0.5, w_t: float = 0.5):
    model_student.train()
    model_teacher.eval()
    criterion.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    ...
    for _ in metric_logger.log_every(range(len(data_loader_label)), print_freq, header):
        ...
        # add to gmm
        gmm.score_update(outputs_unsup_q) # based on the Teacher
        threshold = thresholds = gmm.get_current_gmm_thresholds() # add compute thresholds for each class
        thresholds = torch.tensor(thresholds).to(device)

        # pseudo label filtering
        if  train_setting == 'dvd' or train_setting == 'sdugamod' or train_setting == 'visDrone' or train_setting == 'coco_add_semi' or train_setting == 'voc_semi':
...
def train_one_epoch_burnin(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        outputs = model(samples)
        # add to gmm to collect for semi-supervised stage
        gmm.score_update(outputs)
