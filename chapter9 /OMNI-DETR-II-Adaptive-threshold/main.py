...
def main(args):
    ...
    # Jacob
    # if args.eval:
    #     data_loader_test = DataLoader(dataset_val, 2, sampler=sampler_val,
    #                              drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers,
    #                              pin_memory=True)
    ...
    # Init GMM
    if args.resume and 'thrs' in checkpoint and 'scores' in checkpoint:
        thrs = checkpoint['thrs']
        scores = checkpoint['scores']
        print("[Adaptive Thresholds]: ", thrs)
        gmm = GMM(num_score_samples=100, num_classes=len(thrs)) # we have 10 classes in DVD
        # gmm.overwrite_thresholds(thrs)
        gmm.overwrite_scores(scores)
    else: 
        gmm = GMM(num_score_samples=100, num_classes=10) # we have 10 classes in DVD
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        elif args.dataset_file == "sdugamod_ground" or  args.dataset_file == "sdugamod" or args.dataset_file == "visDrone" or args.dataset_file == "coco_omni" or args.dataset_file == 'coco_add_semi' or args.dataset_file == "coco_35to80_tagsU" or args.dataset_file == "coco_35to80_point" or args.dataset_file == 'coco_objects_tagsU' or args.dataset_file == 'coco_objects_points' or args.dataset_file == 'bees_omni' or args.dataset_file == 'voc_semi' or args.dataset_file == 'voc_omni' or args.dataset_file == 'objects_omni' or args.dataset_file == 'crowdhuman_omni':
            if args.distributed:
        ...
        if epoch < args.BURN_IN_STEP:
            train_stats = train_one_epoch_burnin(
                model_student, criterion_burnin, data_loader_train_burnin, optimizer_burnin, device, epoch, gmm,
                args.clip_max_norm)
